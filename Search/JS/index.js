...
async function searchByName(sn) {
...
function displaySearchByName(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        cartona += `<div class="col-md-3 mt-5">
                        <div onclick='displaySearchData(${arr[i].idMeal})' class="position-relative home-img border-0 rounded-3">
...
function displaySearchByLetter(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        cartona += `<div class="col-md-3 mt-5">
                        <div onclick='displaySearchData(${arr[i].idMeal})' class="position-relative home-img">
...
function displaySearch2(meal) {
    let cartona = `
        <div class="col-md-4">
            <div class="img-Home-search2 mt-5 ">
...
async function start(searchName) {
    let d = await searchByName(searchName);
    if (d) {
...
async function start2(SearchLetter) {
    let d = await searchByLetter(SearchLetter);
    if (d) {
...
function test() {
    searchName.addEventListener('input', function () {
        start(searchName.value);
    });
}

function test2() {
    searchLetter.addEventListener('input', function () {
        start2(searchLetter.value);
    });
}
